...
class labelme2coco(object):
    ...
    def data_transfer(self):
        for num, json_file in enumerate(self.labelme_json):
            with open(json_file, "r") as fp:
                data = json.load(fp)
                self.images.append(
                    self.image(data, os.path.basename(os.path.dirname(json_file)), num))
    ...
    def image(self, data, dir, num):
        ...
        image["width"] = width
        image["id"] = num
        image["file_name"] = os.path.join(dir, data["imagePath"].split("/")[-1])
...
if __name__ == "__main__":
    ...
    labelme_json = []
    for dir in os.listdir(args.labelme_images):
        dir = os.path.join(args.labelme_images, dir)
        if os.path.isdir(dir):
            labelme_json += glob.glob(os.path.join(dir, "*.json"))
