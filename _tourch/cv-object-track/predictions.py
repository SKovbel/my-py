...
def nms_pytorch(P: torch.tensor, labels: torch.tensor, scores: torch.tensor, thresh_iou: float):
    ...
    pred_dict['boxes'], pred_dict['labels'], pred_dict['scores'] = pred_dict['boxes'][0], pred_dict['labels'][0], \
    pred_dict['scores'][0]
    return pred_dict
...
def apply_nms(predictions, threshold):
    ...
    for img in range(len(predictions)):
        nms_list.append(
            nms_pytorch(predictions[img]['boxes'], predictions[img]['labels'], predictions[img]['scores'], threshold))
    return nms_list
