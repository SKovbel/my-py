...
def path(name, mkdir=False):
    DIR = os.path.dirname(os.path.realpath(__file__))
    new_path = os.path.join(DIR, '..', '..', 'tmp', 'speach_recognition', name)
    if mkdir:
...
class CustomSchedule(keras.optimizers.schedules.LearningRateSchedule):
    ...
    def get_config(self):
        # Implement get_config() to make the schedule serializable
        return {
...
keras.utils.get_file(
    ...
    extract=True,
    archive_format="tar",
    cache_dir='.', cache_subdir=path(".", True),
)

saveto = path("LJSpeech-1.1", True)
...
display_cb = DisplayOutputs(batch, vectorizer.get_vocabulary(), target_start_token_idx=2, target_end_token_idx=3)

save_path = path('model_save')
if os.path.exists(save_path):
    #model = load_model(save_path, custom_objects={'CustomSchedule': CustomSchedule})
