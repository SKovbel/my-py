...
class Preparation:
    ...
    def __expand(self, input):
        # scale
        width, height = min(len(input), WIDTH), min(len(input[0]), HEIGHT)
        ...
        for c in range(0, sample.shape[0]):
            for y in range(0, sample.shape[1]):
                for x in range(0, sample.shape[2]):
                    if sample[c][y][x] != 0:
                        print(COLORS[color] if color else COLORS[c])
    ...
    def plot_layers(self, key_id=0, img_id=0):
        ...
        X = data['X_train'][img_id]
        Y = data['y_train'][img_id]
        _, axes = plt.subplots(max(len(X)//2, 1), 4, figsize=(20, 20))
        axes = np.atleast_2d(axes)
        for c in range(0, max(len(X)//2, 1)):
            d = 0 if c % 2 == 0 else 2
    ...
    def plot_sample(self, key_id=0, img_id=0):
        ...
        axes[0].imshow(self.sample_to_image(data['X_train'][img_id]))
        axes[1].imshow(self.sample_to_image(data['y_train'][img_id]))
        plt.show()

    def plot_test(self, key_id=0):
        data = self.samples(key_id)
        _, axes = plt.subplots(1, 2, figsize=(20, 20))
        axes[0].imshow(self.sample_to_image(data['X_test']))
        axes[1].imshow(self.sample_to_image(data['y_test']))
        plt.show()
...
def clone_sample(sample, n):
    return tf.data.Dataset.from_tensors(sample).repeat(n)

# Creating input stream
train_dataset = tf.data.Dataset.from_tensor_slices((X_train, X_train))
train_dataset = train_dataset.flat_map(lambda x, y: clone_sample((x, y), 1000))
        ...
        for i in range(HEIGHT):
            ...
            for j in range(WIDTH):
                for k in range(CHANNEL):
                    ...
                    print(logits)
                    exit(0)
                    logits = tf.reshape(logits, [n_samples, HEIGHT, WIDTH, CHANNEL, -1])
                    print("Logits shape:", logits.shape)
                    
                    logits_sliced = logits[:, i, j, k, :]
                    print(f'logits_sliced shape: {logits_sliced.shape} at position ({i}, {j}, {k})')

                    next_sample = tf.random.categorical(logits_sliced, 1)
                    samples[:, i, j, k] = tf.squeeze(next_sample, axis=-1)
                    print(f'{j}', end=',')
        return samples
...
fig,ax = plt.subplots(1, n_samples, figsize=(15, 10), dpi=80)
for i in range(n_samples):
    ax[i].imshow(prep.sample_to_image(samples[i, :, :, :]))
plt.show()