...
class Preparation:
        ...
        with open(f'{DATA_DIR}/arc-agi_training_challenges.json', 'r') as file:
            ...
            for key in train:
                for index in range(0, len(train[key]['train'])):
                    X_train.append(self.__expand(train[key]['train'][index]['input']))
                    y_train.append(self.__expand(train[key]['train'][index]['output']))
                X_test.append(self.__expand(train[key]['test'][0]['input']))

        y_test = []
        with open(f'{DATA_DIR}/arc-agi_training_solutions.json', 'r') as file:
            test = json.load(file)
            for key in train:
                y_test.append(self.__expand(test[key][0]))
    ...
    def plot_sample(self, index=0):
        ...
        axes[0].imshow(self.sample_to_image(self.X_train[index]))
        axes[1].imshow(self.sample_to_image(self.y_train[index]))
        plt.show()

    def plot_test(self, index=0):
        _, axes = plt.subplots(1, 2, figsize=(20, 20))
        axes[0].imshow(self.sample_to_image(self.X_test[index]))
        axes[1].imshow(self.sample_to_image(self.y_test[index]))
        plt.show()

prep = Preparation()

idx = 22
#prep.plot_layers(idx)
#prep.plot_sample(idx)
#prep.plot_test(1)
