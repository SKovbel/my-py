...
def discretized_mix_logistic_loss(x, l):
    ...
    nr_mix = int(ls[-1] / 10)
    logit_probs, means, log_scales, coeffs = tf.split(l, num_or_size_splits=[nr_mix, 3 * nr_mix, 3 * nr_mix, 3 * nr_mix], axis=-1)
...
def sample_from_discretized_mix_logistic(l, nr_mix):
    ...
    # unpack parameters
    logit_probs = tf.nn.log_softmax(logit_probs - tf.reduce_max(logit_probs, -1, keepdims=True))  # log_prob_from_logits(tf.convert_to_tensor(l[:, :, :, :nr_mix])).numpy()

    # sample mixture indicator from softmax
    sel = tf.argmax(logit_probs - tf.math.log(-tf.math.log(tf.random.uniform(list(logit_probs.shape), minval=1e-5, maxval=1. - 1e-5))), 3)
