...
class SimpleCouplingLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_b = x_b + self.linear(x_a)
        x = torch.cat([x_a, x_b], dim=1)

        if self.SHUFFLE:
            x = self.reverse_shuffle_features(x, perm)

        # Return the output and 0 for log_det_jacobian (as no transformation)
...
class NormalizingFlow(nn.Module):
    def __init__(self, in_features, num_layers=4):
        ...
        self.layers = nn.ModuleList([SimpleCouplingLayer(in_features) for _ in range(num_layers)])
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            log_det_jacobian += _log_det
        return x, log_det_jacobian, layer_outputs

def train(flow_model, X_train, distribution):
...
flow_model = NormalizingFlow(in_features=4, num_layers=5)
base_normal_distribution = MultivariateNormal(torch.zeros(4), torch.eye(4))
train(flow_model, X_train, base_normal_distribution)
