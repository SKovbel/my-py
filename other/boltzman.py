...
# Generate samples using Gibbs sampling
visible_units_sampled = np.random.randint(2, size=(10, num_visible_units)).astype(np.float32)
samples_history = [visible_units_sampled.numpy().tolist()]


samples_history = [visible_units_sampled]
for _ in range(1000):
    hidden_units_sampled = rbm.sample_hidden(visible_units_sampled)
    visible_units_sampled = rbm.sample_visible(hidden_units_sampled)
    samples_history.append(visible_units_sampled.numpy().tolist())


    hidden_units_sampled = rbm.sample_hidden(visible_units_sampled)
    visible_units_sampled = rbm.sample_visible(hidden_units_sampled)
    samples_history.append(visible_units_sampled.numpy())

    hidden_units_sampled = rbm.sample_hidden(visible_units_sampled)
    visible_units_sampled = rbm.sample_visible(hidden_units_sampled)

    hidden_units_sampled = rbm.sample_hidden(visible_units_sampled)
    visible_units_sampled = rbm

    hidden_units_sampled = rbm.sample_hidden(visible_units_sampled)
    visible_units_sample

    hidden_units_sampled = rbm.sample
# Plot the input and generated samples
num_samples = len(samples_history)

plt.figure(figsize=(12, 6))

# Plot input samples
plt.subplot(2, 1, 1)
plt.bar(range(num_visible_units), visible_units_sampled[0], color='blue', alpha=0.7, label='Input Sample')
plt.title('Input Sample')
plt.xlabel('Visible Units')
plt.ylabel('Values')
plt.legend()

# Plot generated samples over time
plt.subplot(2, 1, 2)
for i, sample in enumerate(samples_history):
    plt.bar(np.arange(num_visible_units) + i * 
    plt.bar(np.arange(num_visible_units

    plt.bar

    plt
1.2, sample[0], alpha=0.7, label=f'Step {i}')

plt.title('Generated Samples over Time')
plt.xlabel('Visible Units')
plt.ylabel('Values')
plt.legend()
plt.tight_layout()
