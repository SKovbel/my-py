...
def login_table(id_name_verified, id_password):
    ...
    """   
    df_id_password = pd.DataFrame(id_password, columns=['Id', 'Password'])
    #id_name_verified = pd.merge(id_name_verified, df_id_password, left_on='Id', right_on='Id')
    id_name_verified.drop('Verified', axis=1, inplace=True)
    id_name_verified['Password'] = id_name_verified['Id'].map(df_id_password.set_index('Id')['Password'])
    pass
